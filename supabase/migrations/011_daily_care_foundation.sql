...
AS $$
  SELECT public.current_app_role() IN ('owner', 'admin')
    OR EXISTS (
      SELECT 1 FROM public.staff_classroom_assignments assignment
      WHERE assignment.classroom_id = target_classroom AND assignment.staff_id = auth.uid()
    )
    OR EXISTS (
      SELECT 1
...
AS $$
  SELECT public.current_app_role() IN ('owner', 'admin')
    OR EXISTS (
      ...
      WHERE child.id = target_child AND child.key_person_id = auth.uid()
    )
    OR EXISTS (
      SELECT 1 FROM public.child_guardians guardian
      WHERE guardian.child_id = target_child AND guardian.guardian_id = auth.uid()
    )
    OR EXISTS (
      SELECT 1
      FROM public.classroom_memberships membership
      JOIN public.staff_classroom_assignments assignment
        ON assignment.classroom_id = membership.classroom_id
      WHERE membership.child_id = target_child
...
AS $$
BEGIN
  INSERT INTO public.child_daily_report_revisions
    (report_id, child_id, report_date, previous_data, changed_by)
  VALUES
    (OLD.id, OLD.child_id, OLD.report_date, to_jsonb(OLD), auth.uid());
  NEW.updated_at := now();
  RETURN NEW;
END;
$$;
...
CREATE TRIGGER stamp_daily_report_publication
BEFORE INSERT OR UPDATE ON public.child_daily_reports
FOR EACH ROW EXECUTE FUNCTION public.stamp_daily_report_publication();
CREATE TRIGGER audit_child_daily_report_update
BEFORE UPDATE ON public.child_daily_reports
...
CREATE POLICY "role_scoped_classroom_memberships_read" ON public.classroom_memberships FOR SELECT TO authenticated
...
CREATE POLICY "role_scoped_daily_reports_read" ON public.child_daily_reports FOR SELECT TO authenticated
  USING (
    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM public.child_guardians guardian
        WHERE guardian.child_id = child_daily_reports.child_id AND guardian.guardian_id = auth.uid()
      )
    )
  );
CREATE POLICY "staff_create_daily_reports" ON public.child_daily_reports FOR INSERT TO authenticated
  WITH CHECK (
    public.current_app_role() IN ('owner', 'admin', 'teacher')
    AND updated_by = auth.uid()
    AND (published_at IS NULL OR published_by = auth.uid())
    AND public.can_access_child(child_id)
    AND public.can_access_classroom(classroom_id)
  );
...
CREATE POLICY "owners_read_daily_report_revisions" ON public.child_daily_report_revisions FOR SELECT TO authenticated
...
CREATE INDEX IF NOT EXISTS child_daily_reports_date_class_idx
  ON public.child_daily_reports (report_date DESC, classroom_id);
CREATE INDEX IF NOT EXISTS child_daily_report_revisions_report_idx
  ON public.child_daily_report_revisions (report_id, changed_at DESC);
